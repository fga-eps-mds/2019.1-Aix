...
class Validator:
    ...
    def __init__(self, domain_path, intents_path, stories_path):
        ...
        if os.path.isfile(intents_path):
            ...
            intent_files = [f for f in listdir(intents_path)
                            if isfile(join(intents_path, f))]
    ...
    def verify_intents(self):
        ...
        # Checks if the intents in domain are the same
        # of the ones in the intent files
        for intent in intents_in_domain:
            found = self.search(intents_in_files, intent)
            if not found:
                logger.error('The intent ' + intent + ' is in the domain file\
                              but was'+' not found in the intent files')
            else:
    ...
    def verify_utters(self):
        ...
        for i in range(start, len(domain_lines)):
            ...
            elif (len(s_line.split()) >= 1 and s_line.split()[0] != '-')\
                    and (s_line.endswith(':') and not line.startswith(' ')):
                break
